...
type Tunnel struct {
	...
	ServerPrivateKey string `json:"-"`                            // Never expose server private key
...
// GenerateTunnelCommands exposes the current command generation logic for
// internal callers like recovery, so they stay aligned with the API.
func GenerateTunnelCommands(tunnel *Tunnel) *TunnelCommands {
	return generateTunnelCommands(tunnel)
}
